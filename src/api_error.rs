use crate::{ApiErrorValue, StatusWrapper};
...
/// be a 404 or a 500.
pub struct ApiError<T, E> {
    value: T,
    code: StatusCode,
    _e: PhantomData<fn() -> E>,
}

impl<T, E> ApiError<T, E> {
    ...
    /// set of status codes.
    pub fn into_superset<E2>(self) -> ApiError<T, E2>
    where
        E2: SupersetOf<E>,
    {
        ApiError::new_unchecked(self.value, self.code)
    }

    /// Convert this [`ApiError`] into a new [`ApiError`] with a different value type.
    pub fn map_value<F, U>(self, f: F) -> ApiError<U, E>
    where
        F: FnOnce(T) -> U,
    {
        ApiError::new_unchecked(f(self.value), self.code)
    }
...
impl<T: ApiErrorValue, R> IntoResponse for ApiError<T, R> {
    fn into_response(self) -> Response {
        self.value.into_response_with(self.code)
    }
}

impl<T: Debug, R> Debug for ApiError<T, R> {
...
impl<T: Clone, R> Clone for ApiError<T, R> {
    fn clone(&self) -> Self {
        Self {
...
impl<T: Hash, R> Hash for ApiError<T, R> {
...
impl<T: PartialEq, R> PartialEq for ApiError<T, R> {
    fn eq(&self, other: &Self) -> bool {
        self.value == other.value && self.code == other.code
    }
}

impl<T: Eq, R> Eq for ApiError<T, R> {}
